...
# def distance_matrix():
		...
		def removeDuplicates(connections):
			...
			for c in connections:
				if ((c[0],c[1]) not in nodubConnections) and ((c[1],c[0]) not in nodubConnections):
					nodubConnections.append(c)
			return nodubConnections
		def createGraph(connections, graph):
		...
		connected_nodes,min_distance_from_sample = get_connections()
		nodubConnections=removeDuplicates(connected_nodes)
		graph=[]
		graph=createGraph(nodubConnections,graph)
