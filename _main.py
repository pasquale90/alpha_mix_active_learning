...
# REQUIRES A FIX
    ...
    if (str(args.mode) == "train"):
        ...
        # round 0 accuracy
        trained_epochs = strategy.train(name=str(round)) # train the first round. using the training split.                             # train for the 1st round 
        ...
        if int(round)==0: #not os.path.exists(str(args.train_out)) or
            ...
            accuracy.index.name="rounds"
        else:
            accuracy = pd.read_csv(str(args.train_out),index_col=0)
            accuracy.loc["round_"+str(round), "accuracy"] = acc[0]
        ...
        budget = args.n_query * int(math.pow(args.query_growth_ratio, round - 1))
