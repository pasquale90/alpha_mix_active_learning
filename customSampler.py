    ...
    train_params_pool = {
        ...
        'mini_domain_net-real':
            {'n_epoch': train_args.n_epoch,
             ...
             'lr_decay_epochs': train_args.lr_decay_epochs,
             'train_to_end': train_args.train_to_end,
             'log_dir': './_logs/mini_domain_net-real',
             'n_early_stopping': train_args.n_early_stopping,
             'continue_training': train_args.continue_training},
        'tiny_domain_net-real':
            {'n_epoch': train_args.n_epoch,
...
def load_dataset(train_params, train_args,sub_path):
...
def al_train_sub_experiment(args, train_args, train_params, strategy_name, general_path, seed):
    ...
    load_dataset(train_params, train_args,sub_path)
    '''
    # load network
    ...
    if strategy_name == 'CDALSampling':
        model = CDALModel(net, net_args, handler, train_params, writer, device, round=int(args.curr_round),dataset=str(args.data_name),model='CDALModel', sampling='CDALSampling', weights_dir = WEIGHTS_DIR)
    else:
        model = Training(net, net_args, handler, train_params, writer, device, round=int(args.curr_round),dataset=str(args.data_name), model=train_args.model, sampling=str(args.strategy), weights_dir = WEIGHTS_DIR)
...
# HERE LOAD THE WEIGHTS OF THE PREVIOUS ROUND
    ...
    # round 0 accuracy
    trained_epochs = strategy.train(name='0') # train the first round. using the training split.                             # train for the 1st round 
...
# HERE STORE THE WEIGHTS OF THE CURRENT ROUND && RETURN THE q_idxs
        ...
        resultspath=os.path.join(os.getcwd(),"_queryResults","results_round-"+str(rd)+".csv")
        result_idxs.to_csv(resultspath,index=False,header=False)

        resultspath=os.path.join(os.getcwd(),"_queryResults","results_round-"+str(rd)+".txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--curr_round', type=int, default=1)
